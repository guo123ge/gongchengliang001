...
import type { ComponentType, QuantityResult, ValidationItem } from "../types";
...
export async function exportWord(
  ...
  children.push(table([
    ...
    ...results.map((r) => row([r.name, TYPE_LABELS[r.type] ?? r.type, r.concreteVolume.toFixed(3), r.formworkArea.toFixed(2), r.totalRebarWeight.toFixed(2)])),
  ]));

  children.push(new Paragraph({ text: "三、计算说明", heading: HeadingLevel.HEADING_1 }));
  const noteRows = results.flatMap((r) => (r.notes ?? []).map((note) => row([r.name, note])));
  children.push(table([
    row(["构件", "说明"], true),
    ...(noteRows.length > 0 ? noteRows : [row(["-", "暂无专项计算说明。"])]),
  ]));

  children.push(new Paragraph({ text: "四、钢筋规格汇总", heading: HeadingLevel.HEADING_1 }));
  children.push(table([
    row(["等级-直径", "总长(m)", "重量(kg)"], true),
    ...Object.entries(agg.rebar).map(([k, v]) => row([k, v.length.toFixed(2), v.weight.toFixed(2)])),
  ]));

  children.push(new Paragraph({ text: "五、22G101 平法校验结论", heading: HeadingLevel.HEADING_1 }));
  if (validations.length === 0) {
    children.push(new Paragraph({ text: "暂无校验项。" }));
  } else {
    children.push(table([
