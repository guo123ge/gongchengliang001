...
export async function POST(req: NextRequest) {
  ...
  if (!cfg || !cfg.apiKey) {
    return NextResponse.json({ error: "AI 未配置，请先在设置中填写 API Base URL、Key 与 Model。" }, { status: 400 });
  }
  const { messages, model: overrideModel } = await req.json();
  const payload = {
  ...
  try {
    ...
    if (!resp.ok) {
      return NextResponse.json({ error: data?.error?.message || "AI 接口调用失败", raw: data }, { status: resp.status });
    }
